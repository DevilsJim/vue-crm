...
<script>
import { getLatentUserPager, getBusinessUserPager } from 'api/customerDetail'
export default {
    ...
    fetchData() {
      ...
      if (companyId && companyId !== null) {
        ...
        if (_this.sourceType === '1') {
          getLatentUserPager(_this.formInline).then(response => {
            _this.listData = Object.assign({}, _this.listData, {
            ...
            _this.$message.success('列表更新！')
          }).catch((err) => {
            _this.$message.error(err)
          })
        } else if (_this.sourceType === '2') {
          getBusinessUserPager(_this.formInline).then(response => {
            _this.listData = Object.assign({}, _this.listData, {
              list: response.data.data,
              total: response.data.total,
              start: response.data.start,
