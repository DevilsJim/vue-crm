  ...
  <div class="app-container customer-detail">
    ...
    <el-container height="auto" direction="vertical">
      <el-row style="padding-bottom:16px;">
        <el-col :span="10">
          <h2>{{ businessInfo.bussinessName }}<svg-icon icon-class="edit" class-name="edit" @click="handlerEditBusiness" /></h2>
        </el-col>
        <el-col :span="14" style="text-align: right">
          <el-button size="small" type="primary" @click="releaseBusinessVisible = true">释放</el-button>
          <el-button size="small" type="primary" @click="transferBusinessVisible = true">转移</el-button>
          <el-button size="small" type="primary" @click="followUpVisible = true">跟进</el-button>
          <el-button size="small" type="primary">代客下单</el-button>
        </el-col>
      </el-row>
      <div class="info">
      ...
      >
        <el-table-column label="客户名称" width="220" class-name="company-name" align="center">
          <template slot-scope="scope">
            <router-link v-if="businessInfo.contactType == 1" :to="{path:'/customerManagement/customerDetail/' + scope.row.companyId, query: {ifOwnCompany: businessInfo.ifOwnCompany}}">
              <div class="name">{{ scope.row.companyName }}</div>
            </router-link>
            <router-link v-else-if="businessInfo.contactType == 0" :to="{path:'/customerManagement/resourceCustomerDetail/' + businessInfo.companyId}">
              <div class="name">{{ scope.row.companyName }}</div>
            </router-link>
          </template>

        </el-table-column>
        <el-table-column label="客户类型" prop="companyType" width="100" class-name="tags" align="center">
          <template slot-scope="scope">{{ scope.row.companyType | companyTypeFilter }}</template>
        </el-table-column>
        <el-table-column label="联系人" prop="nickname" width="100" align="center" />
        <el-table-column label="职位" prop="duty" width="100" align="center" />
        <el-table-column label="联系电话" prop="userMobile" width="110" align="center" />
        <el-table-column label="公司地址" prop="licenseAddress" min-width="120" align="center" />
        <el-table-column label="客户负责人" prop="sysUserName" width="100" align="center" />
        <el-table-column label="操作" align="center" width="160" fixed="right">
          <template slot-scope="scope">
            <el-button v-if="scope.row.contactType == '0' && currentIfOwnCompany == '2'" type="primary" plain size="mini" icon="el-icon-plus" @click="handleAddEditContacts(true, scope.row.userId)">捡入到我的客户</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
        ...
        <el-tabs v-model="activeTabsName" type="border-card">
          ...
          <el-tab-pane label="跟进记录" name="first">
            <el-table :data="businessFollowList.list" border>
              ...
              <el-table-column width="120" prop="sysName" label="跟进人" align="center" />
              <el-table-column width="120" label="跟进方式" align="center">
                <template slot-scope="scope">
                  {{ scope.row.followType | followTypeFilter }}
                </template>
              </el-table-column>
              ...
              <el-table-column min-width="200" prop="followMark" label="跟进/备注" />
            </el-table>
            <!-- 分页 -->
            <el-pagination
              background
    ...
    <!-- 编辑商机信息 -->
    <edit-business-dialog :visible.sync="editBusinessVisible" :business-data="editBusinessData" />

    <!-- 释放商机 -->
    <release-business-dialog :visible.sync="releaseBusinessVisible" :business-id="currentBusinessId" :pk-sid="businessInfo.pkSid" />

    <!-- 转移商机 -->
    <transfer-business-dialog :visible.sync="transferBusinessVisible" :business-id="currentBusinessId" :pk-sid="businessInfo.pkSid" />

    <!-- 跟进商机 -->
    <follow-up-dialog :visible.sync="followUpVisible" :follow-up-data="businessInfo" />
  </div>
</template>
...
import ReleaseBusinessDialog from './ReleaseBusinessDialog'
import TransferBusinessDialog from './TransferBusinessDialog'
...
export default {
  ...
  data() {
    return {
      ...
      editBusinessVisible: false, // 编辑商机
      followUpVisible: false, // 跟进商机
      releaseBusinessVisible: false, // 释放商机
      transferBusinessVisible: false // 转移商机
    }
  },
  watch: {
