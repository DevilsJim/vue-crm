<template>
  <div class="customer-list-table">
    ...
    <el-form :inline="true" :model="formInline" size="small">
      ...
      <!-- 顾问 -->
      <el-form-item>
        <el-select v-model="formInline.currentUserId" filterable placeholder="请选择的顾问" clearable>
          <el-option
      ...
      <template v-if="formInline.needApprove != 2">
        <el-form-item>
          <el-select v-model="formInline.contactType" clearable placeholder="请选择客户池">
      ...
      <!-- 客户标签 -->
      <el-form-item>
        <el-select v-model="formInline.companySign" clearable placeholder="请选择客户标签">
      ...
      <!-- 意向业务 -->
      <el-form-item>
        <el-select v-model="formInline.businessPurpose" clearable placeholder="请选择意向业务">
      ...
      <template v-if="formInline.needApprove != 2">
        <el-form-item>
          <el-select v-model="formInline.needApprove" clearable placeholder="是否需审批">
    ...
    >
      ...
      <el-table-column label="客户名称" width="220" class-name="company-name" header-align="center">
        <template slot-scope="scope">
          <router-link :to="{path:'/TeamManagement/customerDetail/' + scope.row.companyId}">
      ...
      <el-table-column label="联系人信息" width="195" class-name="user-info" header-align="center">
        <template slot-scope="scope">
          ...
          <span class="user-phone">电话: {{ scope.row.latentUserMobile }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" header-align="center" class-name="operation" fixed="right" min-width="155">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.contactType === 1" type="primary" size="small" @click="handleChangeCustomer(scope.row.companyId)">变更</el-button>
          <el-button v-else-if="scope.row.contactType === 0" type="primary" size="small" @click="handleAssignAdviser(scope.row.companyId)">分配</el-button>
          <!--          <el-button v-else type="primary" size="small" @click="handleChangeCustomer(scope.row.companyId)">变更</el-button>-->
...
<script>
import { getSysUserPageList } from 'api/teamMeagement'
